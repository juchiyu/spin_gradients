---
title: "3_PLSC_grad_behav_oliver"
author: "Ju-Chi.Yu"
date: "4/20/2022"
...
```{r, echo = FALSE, warning= FALSE, message='hide'}
lol_spins_behav <- 
  read_csv('../data/spins_lolivers_subject_info_for_grads_2022-04-20.csv') %>%
...
spins_behav_num <- lol_spins_behav %>% 
  select(scog_rmet_total, scog_er40_total, scog_mean_ea, scog_tasit1_total,
...
## kept subjects
kept.sub <- behav.sub[behav.sub %in% grad.sub] # 405
# spins_behav_num[complete.cases(spins_behav_num) == FALSE,]
kept.sub[complete.cases(spins_behav_num[kept.sub,]) == FALSE]
kept.sub <- kept.sub[complete.cases(spins_behav_num[kept.sub,])] # 389

## grab the matching data

behav.dat <- spins_behav_num[kept.sub,c(1:15)]
...
spins_demo <- lol_demo %>% 
  select(demo_sex, demo_age_study_entry, diagnostic_group, scog_rmet_total, scog_er40_total, scog_mean_ea, scog_tasit1_total,
...
colnames(spins_demo)
rownames(spins_demo) <- lol_demo$subject

spins_demo_fin <- spins_demo[kept.sub,]
...
cbind(table(spins_demo_fin$diagnostic_group, spins_demo_fin$demo_sex), table(spins_demo_fin$diagnostic_group))
```
...
behav.reg <- apply(behav.dat, 2, function(x) lm(x~regout.dat$scanner + regout.dat$demo_sex + regout.dat$demo_age_study_entry + regout.dat$fd_mean_rest)$residual)

grad.reg <- apply(grad.dat, 2, function(x) lm(x~regout.dat$scanner + regout.dat$demo_sex + regout.dat$demo_age_study_entry + regout.dat$fd_mean_rest)$residual)

grad.reg2plot <- apply(grad.dat, 2, function(x){
  model <- lm(x~regout.dat$scanner + regout.dat$demo_sex + regout.dat$demo_age_study_entry + regout.dat$fd_mean_rest)
...
grad.reg.wide <- grad.reg.long %>% pivot_longer(cols = "sub.CMH0001":"sub.ZHP0171",
             names_to = "Subject",
             names_prefix = "",
             values_to = "value") 
...
G1G2.mean.control$zeMap + ggtitle("Control")
G1G2.mean.case$zeMap + ggtitle("Case")
# 

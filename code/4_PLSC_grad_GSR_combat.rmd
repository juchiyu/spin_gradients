...
table(spins_demo$demo_sex, spins_demo$diagnostic_group)/colSums(table(spins_demo$demo_sex, spins_demo$diagnostic_group)) 
...
## stats test
### mean and sd for all measures - 
###        wtar, parental education, cpz, bprs, sans, bsfs, qls
add_demo_summary <- add_demo %>% 
  gather(key = variable, value = value, -diagnostic_group) %>% 
  group_by(diagnostic_group, variable) %>% 
  summarise(value = list(value)) %>% 
  spread(diagnostic_group, value) %>% 
...
### for measures that can do group comparisons -
###       wtar, parental education, bsfs
add_demo2compare_summary <- add_demo %>% 
  select(diagnostic_group, bsfs_total, edu_mom, edu_dad, wtar_std_score) %>%
  gather(key = variable, value = value, -diagnostic_group) %>% 
  group_by(diagnostic_group, variable) %>% 
  summarise(value = list(value)) %>% 
  spread(diagnostic_group, value) %>% 
...
add_demo2compare_summary$p_FDR <- p.adjust(add_demo2compare_summary$p_value, method = "fdr") 

add_demo2compare_summary
```
...
glasser_brain4plot <- pq_for_plot %>%
  as.data.frame() %>%
   ...
   filter(gradient == "grad1") %>%
  ggplot() +
  geom_brain(mapping = aes(fill = network, size = I(0.001)),
             atlas = glasser,
...
G1G2.roi.control$zeMap_background + G1G2.roi.case$zeMap_dots + G1G2.mean.case$zeMap_dots + G1G2.mean.case$zeMap_text + xlab( "Gradient 1") + ylab("Gradient 2") + ggtitle("Gradients: SSD")
...
```{r}
## Extract statistics
Extr_F <- all_roi_test %>% group_by(Network, gradient) %>%  summarise(Fmin = min(statistic.cor[abs(statistic.cor) > 0]),
                                                            Fmax = max(statistic.cor[abs(statistic.cor) > 0]),
                                                            n = sum(p_FDR < 0.05))
Extr_F
...
G1G2.roi.control$zeMap_background + G1G2.mean.control$zeMap_text + G1G2.arrows.roi + xlab( "Gradient 1") + ylab("Gradient 2") + ggtitle("From Control to SSDs")
...
G2G3.roi.control$zeMap_background + G2G3.mean.control$zeMap_text + G2G3.arrows.roi + xlab( "Gradient 2") + ylab("Gradient 3") + ggtitle("From Control to SSDs")
```
...
G1G2.arrows.roi.sigG1 <- annotate("segment", 
                            x = grad2plot2$grad1.control, 
                            y = grad2plot2$grad2.control, 
                            xend = grad2plot2$grad1.case, 
                            yend = grad2plot2$grad2.case, 
                            color = roi.col$oc[rownames(grad2plot2),], 
                            alpha = spins_cj$grad1[rownames(grad2plot2), "cjsig1"]*100,#0.2, 
                            arrow = arrow(length = unit(0.2, "cm"), 
                                          type = "closed", angle = 20), 
                            linewidth = 0.8)
...
G1G2.roi.control$zeMap_background + G1G2.mean.control$zeMap_text + G1G2.arrows.roi.impG1 + xlab( "Gradient 1") + ylab("Gradient 2") + ggtitle("From control to case\nContribute more than average")
...
brain_sig1 <- brain_sig_grad1dat %>% ggplot() +
  geom_brain(mapping = aes(fill = ROI, size = I(0.001), color = I("grey70")),
             atlas = glasser,
             position = position_brain(hemi~side))+
...
G1G2.mean.control$zeMap_background + G1G2.mean.control$zeMap_text + G1G2.points.pls$zeMap_dots + G1G2.arrows.roi.sigG1 + xlab( "Gradient 1") + ylab("Gradient 2") + ggtitle("From control to case\nSignificant Gradient 1 contribution")+ annotation_custom(ggplotify::as.grob(brain_sig1),-0.10,1.2,0.60,2) 
...
G2G3.roi.control$zeMap_background + G2G3.mean.control$zeMap_text +G2G3.arrows.roi.impG2 + xlab( "Gradient 2") + ylab("Gradient 3") + ggtitle("From control to case\nContribute more than average")
...
G2G3.arrows.roi.sigG2 <- annotate("segment", 
                            x = grad2plot2$grad2.control, 
                            y = grad2plot2$grad3.control, 
                            xend = grad2plot2$grad2.case, 
                            yend = grad2plot2$grad3.case, 
                            color = roi.col$oc[rownames(grad2plot2),], 
                            alpha = spins_cj$grad2[rownames(grad2plot2), "cjsig1"]*100,#0.2, 
                            arrow = arrow(length = unit(0.2, "cm"), 
                                          type = "closed", angle = 20), 
                            linewidth = 0.8)
...
brain_sig2 <- brain_sig_grad2dat %>% ggplot() +
  geom_brain(mapping = aes(fill = ROI, size = I(0.001), color = I("grey70")),
             atlas = glasser,
...
G2G3.roi.control$zeMap_background + G2G3.mean.control$zeMap_text + G2G3.points.pls.G2$zeMap_dots + G2G3.arrows.roi.sigG2 + xlab( "Gradient 2") + ylab("Gradient 3") + ggtitle("From control to case\nSignificant Gradient 2 contribution") +
  # ylim(-0.6, 1) +
  # xlim(-1.1, 1.6) +
  ylim(-1.1, 1) +
  xlim(-1.05, 1.8) +
  annotation_custom(ggplotify::as.grob(brain_sig2),0.5,1.7,0.3,1.1) 
...
G2G3.roi.control$zeMap_background + G2G3.mean.control$zeMap_text + G2G3.arrows.roi.impG3 + xlab( "Gradient 2") + ylab("Gradient 3") + ggtitle("From control to case\nContribute more than average")
...
G2G3.arrows.roi.sigG3 <- annotate("segment", 
                            x = grad2plot2$grad2.control, 
                            y = grad2plot2$grad3.control, 
                            xend = grad2plot2$grad2.case, 
                            yend = grad2plot2$grad3.case, 
                            color = roi.col$oc[rownames(grad2plot2),], 
                            alpha = spins_cj$grad3[rownames(grad2plot2), "cjsig1"]*100,#0.2, 
                            arrow = arrow(length = unit(0.2, "cm"), 
                                          type = "closed", angle = 20), 
                            linewidth = 0.8)
...
brain_sig3 <- brain_sig_grad3dat %>% ggplot() +
  geom_brain(mapping = aes(fill = ROI, size = I(0.001), color = I("grey70")),
             atlas = glasser,
             position = position_brain(side ~ hemi))+
  scale_fill_manual(name = "Networks", 
...
G2G3.roi.control$zeMap_background + G2G3.mean.control$zeMap_text + G2G3.points.pls.G3$zeMap_dots +  G2G3.arrows.roi.sigG3 + xlab( "Gradient 2") + ylab("Gradient 3") + ggtitle("From control to case\nSignificant Gradient 3 contribution") +  
  ylim(-1.1, 1) +
  xlim(-1.05, 1.8) + annotation_custom(ggplotify::as.grob(brain_sig3),0.65,1.95,0,1.1) 
...
```{r, echo = FALSE, warning= FALSE, message='hide'}
subcor.col <- c("grey60", "grey60", "#a6cee3", "#1f78b4", "grey60", "#b2df8a", "grey60", "#B03060", "#ff7f00", "#CD6889", "grey60", "grey60", "#33a02c", "#EE9A49", "#B452CD", "#68228B", "#FFD700", "#D1AE22", rep("grey60", 11)) %>% setNames(aseg$data$label)
...
G1G2.mean.control4subcort <- createFactorMap(net_mean_bygrp[,5:7],
                                     ...
                                     text.cex = 3,
                                     alpha.points = 0.7,
                                     alpha.labels = ifelse(net_mean_bygrp$Network == "Subcortical", 1, 0.3),
                                     col.background = "grey96",
                                     col.axes = "grey30")
...
G1G2.subcorpoints.pls <- createFactorMap(G1G2.subcorpoints2plot.sigG1,
                        axis1 = 1, axis2 = 2,
                        col.points = roi.subcor.idx[rownames(subroi2plot2),"color"],
                        pch = ifelse(spins_cj$grad1[rownames(subroi2plot2), "dir.1"] > 0, 15, 16),
                        cex = 3,
                        alpha.points = spins_cj$grad1[rownames(subroi2plot2), "cjsig1"]*100)
                              

G1G2.arrows.subcorROI.sigG1 <- annotate("segment", 
                            x = subroi2plot2$grad1.control, 
                            y = subroi2plot2$grad2.control, 
                            xend = subroi2plot2$grad1.case, 
                            yend = subroi2plot2$grad2.case, 
                            color = roi.subcor.idx[rownames(subroi2plot2),"color"], 
                            alpha = spins_cj_subcor$grad1[rownames(subroi2plot2), "cjsig1"]*100,#0.2, 
                            arrow = arrow(length = unit(0.2, "cm"), 
                                          type = "closed", angle = 30), 
                            linewidth = 0.8)
...
asegbrain_G1 <- ggseg(atlas="aseg", 
        mapping=aes(fill= label), 
        colour="black", 
        size=.5,
        hemisphere = c("left", "right")) + 
  scale_fill_manual(name = "Regions", values = subcor.col.G1 ) +
  theme(axis.text.y.left = element_blank(), 
        axis.text.x.bottom = element_blank(),
        legend.position = "") + 
...
G1G2.mean.control4subcort$zeMap_background + G1G2.mean.control4subcort$zeMap_text + G1G2.subcorpoints.pls$zeMap_dots + G1G2.arrows.subcorROI.sigG1 + xlab( "Gradient 1") + ylab("Gradient 2") + ggtitle("From control to case\nSignificant Gradient 1 contribution") + annotation_custom(ggplotify::as.grob(asegbrain_G1),0,1.1,0.55,1.75)
...
G2G3.subcorpoints.pls.G2 <- createFactorMap(G2G3.subcorpoints2plot.sigG2,
                        axis1 = 1, axis2 = 2,
                        col.points = roi.subcor.idx[rownames(subroi2plot2),"color"],
                        pch = ifelse(spins_cj$grad2[rownames(subroi2plot2), "dir.1"] > 0, 15, 16),
                        cex = 3,
                        alpha.points = spins_cj$grad2[rownames(subroi2plot2), "cjsig1"]*100)


G2G3.arrows.subcorROI.sigG2 <- annotate("segment", 
                            x = subroi2plot2$grad2.control, 
                            y = subroi2plot2$grad3.control, 
                            xend = subroi2plot2$grad2.case, 
                            yend = subroi2plot2$grad3.case, 
                            color = roi.subcor.idx[rownames(subroi2plot2),"color"], 
                            alpha = spins_cj_subcor$grad2[rownames(subroi2plot2), "cjsig1"]*100,#0.2, 
                            arrow = arrow(length = unit(0.2, "cm"), 
                                          type = "closed", angle = 30), 
                            linewidth = 0.8)
...
asegbrain_G2 <- ggseg(atlas="aseg", 
        mapping=aes(fill= label), 
        colour="black", 
        size=.5,
        hemisphere = c("left", "right")) + 
  scale_fill_manual(name = "Regions", values = subcor.col.G2 ) +
  theme(axis.text.y.left = element_blank(), 
        axis.text.x.bottom = element_blank(),
        legend.position = "") + 
  theme_brain() + theme(text = element_blank())
...
G2G3.mean.control4subcort$zeMap_background + G2G3.mean.control4subcort$zeMap_text + G2G3.subcorpoints.pls.G2$zeMap_dots + G2G3.arrows.subcorROI.sigG2 + xlab( "Gradient 2") + ylab("Gradient 3") + ggtitle("From control to case\nSignificant Gradient 2 contribution") + annotation_custom(ggplotify::as.grob(asegbrain_G2),0.75,1.75,-0.15,0.75)
...
G2G3.subcorpoints.pls.G3 <- createFactorMap(G2G3.subcorpoints2plot.sigG2,
                        axis1 = 1, axis2 = 2,
                        col.points = roi.subcor.idx[rownames(subroi2plot2),"color"],
                        pch = ifelse(spins_cj$grad3[rownames(subroi2plot2), "dir.1"] > 0, 15, 16),
                        cex = 3,
                        alpha.points = spins_cj$grad3[rownames(subroi2plot2), "cjsig1"]*100)

G2G3.arrows.subcorROI.sigG3 <- annotate("segment", 
                            x = subroi2plot2$grad2.control, 
                            y = subroi2plot2$grad3.control, 
                            xend = subroi2plot2$grad2.case, 
                            yend = subroi2plot2$grad3.case, 
                            color = roi.subcor.idx[rownames(subroi2plot2),"color"], 
                            alpha = spins_cj_subcor$grad3[rownames(subroi2plot2), "cjsig1"]*100,#0.2, 
                            arrow = arrow(length = unit(0.2, "cm"), 
                                          type = "closed", angle = 30), 
                            linewidth = 0.8)
...
asegbrain_G3 <- ggseg(atlas="aseg", 
        mapping=aes(fill= label), 
        colour="black", 
        size=.5,
        hemisphere = c("left", "right")) + 
  scale_fill_manual(name = "Regions", values = subcor.col.G3) +
  theme(axis.text.y.left = element_blank(), 
        axis.text.x.bottom = element_blank(),
        legend.position = "") + 
  theme_brain() + theme(text = element_blank())
...
G2G3.mean.control4subcort$zeMap_background + G2G3.mean.control4subcort$zeMap_text + G2G3.subcorpoints.pls.G3$zeMap_dots + G2G3.arrows.subcorROI.sigG3 + xlab( "Gradient 2") + ylab("Gradient 3") + ggtitle("From control to case\nSignificant Gradient 3 contribution") + annotation_custom(ggplotify::as.grob(asegbrain_G3),0.75,1.75,-0.15,0.75)

```

### Merge figures

```{r, echo = FALSE, warning= FALSE, message='hide', out.width='100%'}
G1.sig <- G1G2.mean.control$zeMap_background + G1G2.mean.control$zeMap_text + G1G2.points.pls$zeMap_dots + G1G2.arrows.roi.sigG1 + xlab( "Gradient 1") + ylab("Gradient 2") + ggtitle("From control to case\nSignificant Gradient 1 contribution")
# annotation_custom(ggplotify::as.grob(brain_sig1),-0.10,1.2,0.60,2)  + annotation_custom(ggplotify::as.grob(asegbrain_G1.black),.6,1.2,0.4,0.9)

grid.arrange(grobs = list(G1.sig, brain_sig1, asegbrain_G1.black),
             widths = c(0.5, 0.5),
...
G2.sig <- G2G3.mean.control$zeMap_background + G2G3.mean.control$zeMap_text + G2G3.points.pls.G2$zeMap_dots + G2G3.arrows.roi.sigG2 + xlab( "Gradient 2") + ylab("Gradient 3") + ggtitle("From control to case\nSignificant Gradient 2 contribution") 
# annotation_custom(ggplotify::as.grob(brain_sig2),0.9,1.8,0.15,0.8) + annotation_custom(ggplotify::as.grob(asegbrain_G2.black),0.75,1.75,-0.15,0.75)
...
grid.arrange(grobs = list(G2.sig, brain_sig2_plot, asegbrain_G2.black),
             widths = c(0.05, 0.7, 0.2, 0.05),
             heights = c(0.75, 0.25),
             layout_matrix = rbind(c(1,1,1,1),
                                   c(NA,2,3,NA)))

G3.sig <- G2G3.mean.control$zeMap_background + G2G3.mean.control$zeMap_text + G2G3.points.pls.G3$zeMap_dots + G2G3.arrows.roi.sigG3 + xlab( "Gradient 2") + ylab("Gradient 3") + ggtitle("From control to case\nSignificant Gradient 3 contribution")
# annotation_custom(ggplotify::as.grob(brain_sig3),0.9,1.8,0.15,0.8) + annotation_custom(ggplotify::as.grob(asegbrain_G3.black),0.75,1.75,-0.15,0.75)
...
```{r, echo = FALSE}
behav_sympt <- read.csv("../data/spins_behav_data_full_05-31-2022.csv") %>%
...
lol_spins_behav_ssd <- lol_spins_behav_smp[lol_spins_behav_smp$diagnostic_group == "case" & rownames(lol_spins_behav_smp) != "SPN01_ZHP_0150",]
# lol_spins_behav_ssd <- lol_spins_behav_ssd[complete.cases(lol_spins_behav_ssd),] 
# removing four people with missing qls
...
## replace people with NAs in SANS subscores as NAs
spins_getsans_ssd <- lol_spins_behav_ssd %>% 
  select(sans_sub_affective_flat_blunt, sans_sub_alogia, sans_sub_avolition_apathy, sans_sub_asocial_anhedonia)

complete.sans <- complete.cases(spins_getsans_ssd)

spins_symp_ssd[!complete.sans,"sans_total_sc"] <- NA
...
cor.res.total <- data.frame(round(cor.mat$ci[,1:3], 2),
# round(c(cor.mat$p[c(1:4),1], cor.mat$p[c(1:4), 2]),4),
round(c(cor.mat$p.adj[c(1:4),1], cor.mat$p.adj[c(1:4), 2]),4))
...
## descriptive stats
cbind(Mean = round(colMeans(spins_symp_ssd_indiv, na.rm = TRUE),2),
      SD = round(apply(spins_symp_ssd_indiv, 2, sd, na.rm = TRUE),2))

apply(spins_symp_ssd_indiv, 2, function(x) sum(is.na(x)))
```



```{r, echo = FALSE, warning= FALSE, message='hide'}
corrplot::corrplot(cor(cbind(spins_symp_ssd_indiv, ssd.lx[,1], ssd.ly[,1])), method = "shade")
...
#spins_symp_ssd_indiv, 
LV.dat <- data.frame(lxly.12)
cpz.clean <- cpz[rownames(spins_symp_ssd_indiv),]
cpz.clean.complete <- cpz.clean[complete.cases(cpz.clean),]

spins_symp_ssd4cor <- spins_symp_ssd[rownames(cpz.clean.complete),]

LV.dat4cor <- LV.dat[rownames(cpz.clean.complete),]

## regress out cpz
spins_symp_ssd_noCPZ <- spins_symp_ssd4cor
  
CPZregress <- apply(spins_symp_ssd4cor, 2, function(x){
  lm(x~cpz.clean.complete$cpz_eq_total)$residuals
})

for (i in 1:ncol(spins_symp_ssd_noCPZ)){
  spins_symp_ssd_noCPZ[names(CPZregress[[i]]), i] <- CPZregress[[i]]
}

## test with cpz
noCPZ.cormat <- psych::corr.test(spins_symp_ssd_noCPZ, LV.dat4cor, adjust = "fdr")

noCPZ.cor.res.total <- data.frame(round(noCPZ.cormat$ci[,1:3], 2),
# round(c(noCPZ.cormat$p[c(1:4),1], noCPZ.cormat$p[c(1:4), 2]),4),
round(c(noCPZ.cormat$p.adj[c(1:4),1], noCPZ.cormat$p.adj[c(1:4), 2]),4))


#=========Other plots================================
cor(data.frame(lm(spins_symp_ssd$`Birchwood Social Functioning Scale ` ~ cpz.clean$cpz_eq_total)$residuals), LV.dat$Behavioural.scores.Dim1)

lm(LV.dat$Behavioural.scores.Dim1 ~ cpz.clean$cpz_eq_total) %>% summary() # n.s.
lm(LV.dat$Brain.scores.Dim1 ~ cpz.clean$cpz_eq_total) %>% summary() # n.s.


## behavioral scores
lm(spins_symp_ssd$`Birchwood Social Functioning Scale ` ~ LV.dat$Behavioural.scores.Dim1 + cpz.clean$cpz_eq_total) %>% summary() # n.s.
```
...
# cor.res$items <- c("Social engagement/withdrawn", "Interpersonal communication/relationship", "Interpersonal prosocial activity", "Recreation activity", "Independence-performance", "Independence-competence", "Interpersonal", "Instrumental role", "Intrapsychic foundations", "Common objects and activities", "Negative symptoms", "Positive symptoms", "Anxiety/depression", "Activation", "Hostility", "Affective flattening or blunting", "Alogia", "Avolition/Apathy", "Asociality/Anhedonia")
...
# cor.res4plot <- t(cor.res) %>% as.data.frame
# 
# ## Radar plot
# radarchart(cor.res4plot[c("Max.", "Min.", "Behaviour_r", "Brain_r", "zero", "sig.neg", "sig.pos"),],
#            axistype = 4,
#            axislabcol = "gray20",
#            centerzero = FALSE,
#            vlcex = 0.5,
...
#            pty = c(16, 16, 32, 32, 32), # point shape
#            pcol = c("firebrick", "royalblue3", "gray20", "gray60", "gray60"),        # Color of the line
#            plwd = c(2, 2, 10, 10, 10),        # 48 Width of the line
#            plty = 1)
...
# dat <- tibble(
#   regions = factor(rownames(cor.res)),
...
cor.res %>%
  # arrange(items) %>%
  ggplot(aes(x = items, y = r, color = var1, group = var1)) + 
  geom_point(color = NA, size = 3) +
  geom_line(aes(x = x, y = y, color = NA, group = NA), data = tibble(x = c(1:19), y = 0), color = "black") +
  geom_line(aes(x = x, y = y, color = NA, group = NA), data = tibble(x = c(19, 1), y = 0), color = "black") +
  geom_ribbon(
    aes(x, ymin = ymin, ymax = ymax), inherit.aes = FALSE, 
    data = tibble(x = 1:19, ymin = -0.13, ymax = 0.13),
    fill = "grey40", alpha = 0.5) + 
  geom_ribbon(
    aes(x, ymin = ymin, ymax = ymax), inherit.aes = FALSE, 
    data = tibble(x = c(1, 19), ymin = -0.13, ymax = 0.13),
    fill = "grey40", alpha = 0.5) + 
  geom_ribbon(
    aes(x, ymin = ymin, ymax = ymax), inherit.aes = FALSE, 
  ...
  scale_color_manual(values = c("Behavioural scores" = "darkolivegreen4", "Brain scores" = "darkorchid4")) +
  labs(y = "", color = "Latent\nvariables")  +
  ylim(-0.5, 0.4) +
  annotate('text', x = 0, y = c(-0.5, -0.25, 0, 0.25, 0.5), label = c(-.5, -.25, 0, .25, .5), size = 9*5/14) +
  theme(axis.text.x = element_text(size = 7, vjust = 0.5, face = "bold",
                                   angle = word_angle,
...
```{r, include = FALSE}
# save.image(file = "data/pls_grad_combat_res.rda")
```

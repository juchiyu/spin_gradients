...
```{r}
pls.res <- tepPLS(behav.dat, grad.dat, make_design_nominal = TRUE, graphs = FALSE)
...
PrettyBarPlot2(pls.res$TExPosition.Data$pdq$p[,4],
               threshold = 0, color4bar = behav.dx$type.col, horizontal = FALSE, main = "Loadings - behavioural")
```
...
full.pq <- data.frame(grad.dx[,1:4], pls.res$TExPosition.Data$pdq$q[,1:4], colMeans(grad.dat))
...
pq_for_plot %>%
  ...
  geom_brain(mapping = aes(fill = q1),
             atlas = glasser) +
  facet_wrap(~gradient, ncol = 1) +
  scale_fill_distiller(palette = "RdBu", limits = c(-0.11,0.11), values = c(0, 0.35, 0.5, 0.65, 1)) +

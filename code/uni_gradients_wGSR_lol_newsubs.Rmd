...
lol_spins_behav <- 
  ...
  mutate(subject = str_c("sub-", str_sub(record_id, 7,9), str_sub(record_id, 11, -1))) %>%
  select(subject, scanner, diagnostic_group, demo_sex, demo_age_study_entry, 
         scog_rmet_total, scog_er40_total, scog_mean_ea, scog_tasit1_total,
...
```{r eval = FALSE}
# dropping this b/c task emp fd mean not in current sheet
...
```{r}
library(table1)
# diagnostic_group + demo_age_study_entry + demo_sex + fd_mean + scanner
lol_spins_behav %>%
  drop_na(fd_mean_rest) %>%
  mutate(site = str_sub(scanner, 1,2)) %>%
  semi_join(subject_within_dispersion, by = c("subject" = "Subject")) %>%
  select(diagnostic_group, demo_age_study_entry, demo_sex, `fd_mean_rest`, site) %>%
  distinct() %>%
  table1(~ demo_age_study_entry + demo_sex + site + `fd_mean_rest` | diagnostic_group, data = .)
```
...
results_subject_within_dispersion <- subject_within_dispersion %>%
  ...
  do(tidy(lm(within_dispersion ~ diagnostic_group + demo_age_study_entry + demo_sex + fd_mean_rest + scanner, data = .))) %>%
  ungroup() %>%
  group_by(term) %>%
...
net_centroids_results <- subject_level_centers %>%
  ...
  do(tidy(lm(grad_value ~ diagnostic_group + demo_age_study_entry + demo_sex + fd_mean_rest + scanner, data = .))) %>%
  ungroup() %>%
  group_by(term) %>%
  mutate(p_FDR = p.adjust(p.value, method = "fdr"))
...
all_roi_results %>%
  ...
  filter(term == "diagnostic_groupcontrol",
         Task == "RS") %>%
  mutate(statistic_sig = if_else(p_FDR < 0.05, statistic, 0),
         roi = str_remove(ROI, "_ROI"),
         label = case_when(str_starts(ROI, "L") ~ str_c("lh_", roi),
                           str_starts(ROI, "R_") ~ str_c("rh_", roi))) %>%
  filter(Network != "Subcortical") %>%
  filter(ROI != "L_10pp_ROI") %>%
  as.data.frame() %>%
  ggplot() +
  geom_brain(mapping = aes(fill = statistic_sig),
        atlas = glasser) +

...
spins_grads_num <- data.frame(spins_grads[,c(3:5, 13,15,16)])
...
rm(spins_grads_wide, spins_grads_wide_org, lol_spins_behav, spins_behav_num)
...
spins_mean <- getMeans(spins_grads_num[,1:3], spins_grads_num$ROI)
grad2plot <- spins_mean
colnames(grad2plot) <- c(paste0("Gradient ", c(1:3)))
grad4grp <- unique(spins_grads_num[,c(4,6)])

...
## data for arrows
grad_fjxgrad <- data.frame(pls.res$TExPosition.Data$fj[,1:3], grad.dx[,c("ROI", "gradient")])
...
G1G2.arrows.dim1 <- annotate("segment", x = net_mean$`Gradient 1`, y = net_mean$`Gradient 2`, xend = net_mean$`Gradient 1` + net_mean.fj$Dim1.grad1, yend = net_mean$`Gradient 2` + net_mean.fj$Dim1.grad2, 
        color = "black", alpha = 0.5, arrow = arrow(length = unit(0.3, 
            "cm"), type = "closed", angle = 20), size = 1.2)
...
G1G2$zeMap_background + G1G2$zeMap_dots + G1G2.mean$zeMap_text + G1G2.arrows.dim1 + G1G2.mean$zeMap_dots
G2G3$zeMap_background + G2G3$zeMap_dots + G2G3.mean$zeMap_text + G2G3.arrows.dim1 + G2G3.mean$zeMap_dots
G1G3$zeMap_background + G1G3$zeMap_dots + G1G3.mean$zeMap_text + G1G3.arrows.dim1 + G1G3.mean$zeMap_dots
```

### Dimension 2

```{r, echo = FALSE, fig.show='hold', out.width='50%'}
G1G2$zeMap_background + G1G2$zeMap_dots + G1G2.mean$zeMap_text + G1G2.arrows.dim2 + G1G2.mean$zeMap_dots
G2G3$zeMap_background + G2G3$zeMap_dots + G2G3.mean$zeMap_text + G2G3.arrows.dim2 + G2G3.mean$zeMap_dots
G1G3$zeMap_background + G1G3$zeMap_dots + G1G3.mean$zeMap_text + G1G3.arrows.dim2 + G1G3.mean$zeMap_dots
```

### Dimension 3

```{r, echo = FALSE, fig.show='hold', out.width='50%'}
G1G2$zeMap_background + G1G2$zeMap_dots + G1G2.mean$zeMap_text + G1G2.arrows.dim3 + G1G2.mean$zeMap_dots
G2G3$zeMap_background + G2G3$zeMap_dots + G2G3.mean$zeMap_text + G2G3.arrows.dim3 + G2G3.mean$zeMap_dots
G1G3$zeMap_background + G1G3$zeMap_dots + G1G3.mean$zeMap_text + G1G3.arrows.dim3 + G1G3.mean$zeMap_dots
```